...
exports.writeAdvice = async (req, res, next) => {
    ...
    try {
        ...
        const advice = await Advice.create({ content, writer: user.id }, { transaction });
        ...
        const promises = [];
        let emotions = new Set();
        for (const post of posts) promises.push(post.getEmotions());
        const postEmotions = await Promise.all(promises);
        for (const postEmotion of postEmotions) {
            for (const emotion of postEmotion) emotions.add(emotion.type);
        }
        emotions = [...emotions];
        ...
        return res.status(200).json({ adviceId: advice.id, content: advice.content, emotions });
    } catch (error) {
...
exports.modifyAdviceContent = async (req, res, next) => {
    ...
    try {
        ...
        const advice = await Advice.findOne({ where: { id: adviceId } });
        // ※ 아래 4절: advice null 체크(404) 부재 — 별도 개선 권장
        ...
        return res.status(200).json({ adviceId, newContent });
    } catch (error) {
        if (transaction) await transaction.rollback();
        return next(error);
    }
...
exports.deleteAdvice = async (req, res, next) => {
    ...
    try {
        ...
        transaction = await sequelize.transaction();
        await Advice.destroy({ where: { id: adviceId }, transaction });
        await transaction.commit();

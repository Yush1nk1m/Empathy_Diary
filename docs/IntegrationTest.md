...
본 섹션은 [cr-03](./API.md#cr-03-ai-챗봇과의-최근-대화-내용-불러오기)에 대한 통합 테스트의 기술이다.

| Method | API URI | Test Name | Test Case ID | Description | Expected Result |
| :--: | :-- | :-- | :--: | :-- | :-- |
| GET | /chatrooms | 로그인되지 않은 상태에서 대화 불러오기 요청 | crit-03-1 | 즉시 대화 불러오기 요청을 보낸다. | 상태 코드 403 응답 |
| GET | /chatrooms | 채팅방이 존재하지 않는 상태에서 대화 불러오기 요청 | crit-03-2 | agent가 어떤 채팅방도 생성하지 않은 상태에서 대화 불러오기 요청을 보낸다. | 상태 코드 404 응답 |
| GET | /chatrooms | 성공적인 대화 불러오기 요청 | crit-03-3 | agent는 채팅방을 2개 생성하고 채팅을 1개 보낸다. agent2는 채팅방을 1개 생성하고 채팅을 2개 보낸다. 이후 agent, agent2는 각각 대화 불러오기 요청을 보낸다. | 모두 상태 코드 200 및 최근의 대화 내역 응답 |
